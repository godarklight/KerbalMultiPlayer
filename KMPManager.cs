    ...
    public class KMPManager : MonoBehaviour
    {
        ...
        public void updateStep()
        {
            try
            {
                ...
                if (HighLogic.CurrentGame.flightState.universalTime < Planetarium.GetUniversalTime())
                    HighLogic.CurrentGame.flightState.universalTime = Planetarium.GetUniversalTime();
                ...
                if (HighLogic.fetch.log.Count > 500 && isInFlight && !syncing)
                {
                    ...
                    {
                        if (logEntry.condition.Contains("NullReferenceException"))
                            nreCount++;
                        if (nreCount >= 25)
                        {
                            forceResync = true;
        ...
        private void checkRemoteVesselIntegrity()
        {
            try
            {
                if (!isInFlight || syncing || warping || docking)
                    return;
                foreach (Vessel vessel in FlightGlobals.Vessels.FindAll(v => v.loaded && v.id != FlightGlobals.ActiveVessel.id && serverVessels_PartCounts.ContainsKey(v.id) && serverVessels_ProtoVessels.ContainsKey(v.id)))
                {
                    if (serverVessels_PartCounts[vessel.id] > 0 && serverVessels_PartCounts[vessel.id] > vessel.Parts.Count)
                    {
                        ...
                        foreach (Part part in serverVessels_Parts[vessel.id])
                        {
                            try
                            {
                                if (!part.vessel.isEVA && part.vessel.id != FlightGlobals.ActiveVessel.id)
                                    killVessel(part.vessel);
                            }
                            catch (Exception e)
                            {
                                Log.Debug("Exception thrown in checkRemoteVesselIntegrity(), catch 1, Exception: {0}", e.ToString());
                            }
                        }
        ...
        private void sendRemoveVesselMessage(Vessel vessel, bool isDocking = false)
        {
            ...
            if (vessel == null || vessel.id.ToString() == SYNC_PLATE_ID)
                return;
        ...
        private KMPVesselUpdate getVesselUpdate(Vessel vessel, bool forceFullUpdate = false, bool idOnlyUpdate = false)
        {
            ...
            if (isInSafetyBubble(vessel.GetWorldPos3D(), vessel.mainBody, vessel.altitude))
                update.clearProtoVessel();
            ...
            //Set privacy lock
            if (serverVessels_IsPrivate.ContainsKey(vessel.id))
                update.isPrivate = serverVessels_IsPrivate[vessel.id];
            else
            ...
            if (isInFlightOrTracking || syncing)
            {
                if (vessel_update.relativeTo == Guid.Empty && (isInFlight && vessel_update.id != FlightGlobals.ActiveVessel.id || (serverVessels_InUse[vessel_update.id] || (serverVessels_IsPrivate[vessel_update.id] && !serverVessels_IsMine[vessel_update.id]))))
                {
                    ...
                    if (!vessel_update.id.Equals(Guid.Empty))
                    {
                        ...
                        if (extant_vessel == null)
                            extant_vessel = FlightGlobals.Vessels.Find(v => v.id == vessel_update.id);
                        if (isInFlight)
                        {
...
//						{
                        ...
                        if (vessel_update.relTime != RelativeTime.PRESENT)
                            incomingDistance = 3000f; //Never treat vessels from another time as close by
                                ...
                                if (extant_vessel.vesselType != VesselType.Flag) //Special treatment for flags
                                {
                                    ...
                                    if (isInFlight)
                                    {
                                        if (!extant_vessel.loaded)
                                        {
                                            if (KMPVessel.situationIsOrbital(vessel_update.situation))
                                                ...
                                                ourDistance = Vector3.Distance(oldPosition, FlightGlobals.ship_position);
                                        }
                                        else
                                            ourDistance = Vector3.Distance(extant_vessel.GetWorldPos3D(), FlightGlobals.ship_position);
                                    }
                                    ...
                                    if ((vessel_update.getProtoVesselNode() != null && (!KMPVessel.situationIsOrbital(vessel_update.situation) || ourDistance > 2500f || extant_vessel.altitude < 10000d)) || countMismatch)
                                    {
                                        ...
                                        if (protovessel != null)
                                        {
                                            ...
                                            if (vessel_update.situation == Situation.FLYING)
                                                serverVessels_LoadDelay[vessel.id] = UnityEngine.Time.realtimeSinceStartup + 5f;
                                        }
                                        else
                                        {
                                            Log.Debug("Protovessel missing!");
                                        }
                                    }
                                    else
                                    {
                                                ...
                                                if (!KMPVessel.situationIsOrbital(vessel_update.situation) || extant_vessel.altitude < 10000f || vessel_update.id == FlightGlobals.ActiveVessel.id || ourDistance > 2500f)
                                                {
                                                    ...
                                                    if (extant_vessel.loaded)
                                                    {
                                                        if (update_body.GetAltitude(vessel.worldPosition) < 10000d)
                                                        {
                                                            ...
                                                            if (new_srf_vel.sqrMagnitude > 1d)
                                                                extant_vessel.ChangeWorldVelocity((-1 * extant_vessel.srf_velocity) + new_srf_vel);
                                                            else
                                                                extant_vessel.ChangeWorldVelocity(-0.99f * extant_vessel.srf_velocity);
                                                        }
                                                        else
                                                        {
                                                            ...
                                                            if (new_obt_vel.sqrMagnitude > 1d)
                                                                extant_vessel.ChangeWorldVelocity((-1 * extant_vessel.obt_velocity) + new_obt_vel);
                                                            else
                                                                extant_vessel.ChangeWorldVelocity(-0.99f * extant_vessel.obt_velocity);
                                                        }
                                                    }
...
//						}
                    ...
                    if (isInFlight && vessel_update.id == FlightGlobals.ActiveVessel.id)
                    {
						    ...
						    )
                        {
                            ...
                            else if (updateFrom != null && updateFrom.loaded)
                            {
                                ...
                                if (!serverVessels_SkippedRendezvousUpdates.ContainsKey(updateFrom.id))
                                ...
                                if (applyUpdate)
                                {
                                    ...
                                    if (diffVel.sqrMagnitude > 0.0025d && diffVel.sqrMagnitude < 2500d)
                                    {
                                        ...
                                        if (updateFrom.packed)
                                            updateFrom.GoOffRails();
                                    ...
                                    Log.Debug("Ignored docking position update: unexpected large pos/vel shift");
                            ...
                            Log.Debug("Ignored docking position update: " + (FlightGlobals.ActiveVessel.altitude > 10000d) + " " + (vessel_update.relativeTo != Guid.Empty) + " " + (Math.Abs(Planetarium.GetUniversalTime() - vessel_update.tick) < 1d));
                    }
                }
            }
        }
        ...
        private ProtoVessel syncOrbit(KMPVessel kvessel, double fromTick, ProtoVessel protovessel, double LAN)
        {
            ...
            if (newOrbit.referenceBody == null)
                newOrbit.referenceBody = FlightGlobals.Bodies.Find(b => b.name == "Sun");
        ...
        private bool syncExtantVesselOrbit(KMPVessel kvessel, double fromTick, Vessel extant_vessel, double LAN)
        {
            ...
            foreach (ManeuverNode mNode in victim.patchedConicSolver.maneuverNodes)
            {
                if (mNode.attachedGizmo != null)
                {
                    ...
                    if (mGizmo.handleAntiNormal.Drag)
                    {
                        victimAvailable = false;
                        break;
                    }
                    if (mGizmo.handleNormal.Drag)
                    {
                        victimAvailable = false;
                        break;
                    }
                    if (mGizmo.handlePrograde.Drag)
                    {
                        victimAvailable = false;
                        break;
                    }
                    if (mGizmo.handleRadialIn.Drag)
                    {
                        victimAvailable = false;
                        break;
                    }
                    if (mGizmo.handleRadialOut.Drag)
                    {
                        victimAvailable = false;
                        break;
                    }
                    if (mGizmo.handleRetrograde.Drag)
                    {
                        victimAvailable = false;
                        break;
                    }
...
//				Log.Debug("body:" + newOrbit.referenceBody.name);
        ...
        {
            if (vessel_id == FlightGlobals.ActiveVessel.id && (serverVessels_InUse.ContainsKey(vessel_id) ? !serverVessels_InUse.ContainsKey(vessel_id) : false))
                return;
            if (serverVessels_LoadDelay.ContainsKey(vessel_id) ? serverVessels_LoadDelay[vessel_id] >= UnityEngine.Time.realtimeSinceStartup : false)
                return;
                ...
                if (oldVessel != null)
                {
                    ...
                    if (protovessel.vesselType != VesselType.EVA && serverVessels_Parts.ContainsKey(vessel_id))
                    {
                        ...
                        foreach (Part part in serverVessels_Parts[vessel_id])
                        {
                            try
                            {
                                if (part.vessel != null && part.vessel.id != oldVessel.id)
                                    killVessel(part.vessel);
                            }
                            catch (Exception e)
                            {
            ...
            {
                Log.Debug("Exception thrown in addRemoteVessel(), catch 2, Exception: {0}", e.ToString());
            }
            try
            {
				...
				else if (protovessel.vesselType == VesselType.Debris && protovessel.situation == Vessel.Situations.SUB_ORBITAL)
                    return;
...
//						id = (KMPCommon.ClientInteropMessageID)id_int;
        ...
        private void krakensBaneWarp(double krakensTick = 0)
        {
                        ...
                        if (FlightGlobals.ActiveVessel.orbit.EndUT > 0)
                        {
						       ...
						       && FlightGlobals.ActiveVessel.orbit.nextPatch != null)
                            {
                                ...
                                if (FlightGlobals.ActiveVessel.orbit.referenceBody == null)
                                    FlightGlobals.ActiveVessel.orbit.referenceBody = FlightGlobals.Bodies.Find(b => b.name == "Sun");
                                Log.Debug("updated to next patch");
                            }
                        }
                        try
            ...
            {
                Log.Debug("Exception thrown in krakensBaneWarp(), catch 1, Exception: {0}", e.ToString());
                Log.Debug("error during sync: " + e.Message + " " + e.StackTrace);
            }
        }
